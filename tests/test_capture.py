...
class TestTickWriterBooks:
    def test_add_book_then_flush_creates_parquet(self, tmp_path: Path) -> None:
        ...
        writer.flush()
        path = tmp_path / "BTCUSDT" / "books" / "2024-06-01.parquet"
        assert path.exists()
    ...
    def test_flush_writes_one_row(self, tmp_path: Path) -> None:
        ...
        writer.add_book(_snap())
        writer.flush()
        df = pl.read_parquet(tmp_path / "BTCUSDT" / "books" / "2024-06-01.parquet")
        assert len(df) == 1

    def test_flush_appends_on_second_flush(self, tmp_path: Path) -> None:
        writer = TickWriter(tmp_path, book_depth=1)
    ...
    def test_midnight_split_writes_two_files(self, tmp_path: Path) -> None:
        writer = TickWriter(tmp_path, book_depth=1)
        ...
        writer.flush()
        assert (tmp_path / "BTCUSDT" / "books" / "2024-05-31.parquet").exists()
        assert (tmp_path / "BTCUSDT" / "books" / "2024-06-01.parquet").exists()
...
class TestTickWriterTrades:
    def test_trade_flush_creates_parquet(self, tmp_path: Path) -> None:
        ...
        writer.flush()
        assert (tmp_path / "BTCUSDT" / "trades" / "2024-06-01.parquet").exists()
    ...
    def test_trade_row_taker_sign_buy(self, tmp_path: Path) -> None:
        ...
        writer.add_trade(_trade())
        writer.flush()
        df = pl.read_parquet(tmp_path / "BTCUSDT" / "trades" / "2024-06-01.parquet")
...
class TestTickWriterMarkPrice:
    def test_mark_price_flush_creates_parquet(self, tmp_path: Path) -> None:
        ...
        writer.flush()
        assert (tmp_path / "BTCUSDT" / "mark_price" / "2024-06-01.parquet").exists()

    def test_mark_price_row_contains_funding_rate(self, tmp_path: Path) -> None:
        writer = TickWriter(tmp_path)
        writer.add_mark_price(_mark())
        writer.flush()
        df = pl.read_parquet(tmp_path / "BTCUSDT" / "mark_price" / "2024-06-01.parquet")
...
class TestSymbolCapture:
    ...
    async def test_on_mark_price_tags_symbol(self, tmp_path: Path) -> None:
        ...
        assert mark_data["symbol"] == "BTCUSDT"
    ...
    async def test_on_book_passes_snapshot_to_writer(self, tmp_path: Path) -> None:
        ...
        await cap._on_book(_snap(ts_ms=1_717_200_000_000))
        assert len(writer._books["BTCUSDT"]) == 1
